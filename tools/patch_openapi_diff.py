...
def main() -> int:
    ...
    if not inserted:
        raise SystemExit("STOP: failed to insert --openapi-url-env (could not detect end of --live-url add_argument).")
    ...
    while idx != -1:
        ...
        if ("args.out" in chunk) and ("missing" in chunk) and ("extra" in chunk) and ("present" in chunk):
            raw3 = raw3[:idx] + '    print(f"OK {args.out}: {len(missing)} missing, {len(extra)} extra, {len(present)} ok")\n' + raw3[end+2:]
            replaced_print = True
            break
