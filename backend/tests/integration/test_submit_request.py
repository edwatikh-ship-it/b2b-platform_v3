...
def test_submit_request_ok():
    with TestClient(app) as client:
        ...
        resp = client.post(f"/apiv1/user/requests/{request_id}/submit")
...
def test_submit_request_invalid_state_400():
    with TestClient(app) as client:
        ...
        r1 = client.post(f"/apiv1/user/requests/{request_id}/submit")
        assert r1.status_code == 200

        r2 = client.post(f"/apiv1/user/requests/{request_id}/submit")
        assert r2.status_code == 400


def test_submit_request_404():
    with TestClient(app) as client:
        r = client.post("/apiv1/user/requests/99999999/submit")
