...
# HANDOFF РІР‚вЂќ B2B Platform (backend bootstrap)

## 0) SSoT / Rules
- SSoT API contract: `api-contracts.yaml` (OpenAPI). All endpoints/DTO must follow it. [api-contracts.yaml] [api-contracts.yaml]
- Architecture is strict: `transport/` (FastAPI routes + DTO only) РІвЂ вЂ™ `usecases/` (business scenarios) РІвЂ вЂ™ `domain/` (pure models/rules) РІвЂ вЂ™ `adapters/` (DB/SMTP/etc). No FastAPI/SQLAlchemy in `domain`/`usecases`. [PROJECT-RULES.md]
...
- Health endpoint:
  - GET `http://localhost:8000/api/v1/health` РІвЂ вЂ™ 200 `{"status":"ok"}` [api-contracts.yaml]
- Swagger UI:
...
- `Invoke-RestMethod http://localhost:8000/api/v1/health` РІвЂ вЂ™ `status = ok` [api-contracts.yaml]
...
## 5) Alembic (migrations) РІР‚вЂќ final working state
...
- `POST /user/requests` (CreateRequestManualRequest РІвЂ вЂ™ CreateRequestResponse) [api-contracts.yaml]
...
### Entries
- 2025-12-13 22:40 MSK РІР‚вЂќ Verified API up: GET /api/v1/health -> 200 {"status":"ok"}; Swagger /docs and /openapi.json available. Verify: open URLs or `Invoke-RestMethod http://localhost:8000/api/v1/health`. 
- 2025-12-13 22:55 MSK РІР‚вЂќ Reset Postgres clean: dropped/recreated DB b2b_platform and role b2b_user; switched DATABASE_URL to 127.0.0.1; granted schema public CREATE/USAGE to b2b_user. Verify: `psql -U b2b_user -h 127.0.0.1 -d b2b_platform` works; `\dt` shows empty baseline before migrations.
- 2025-12-13 23:00 MSK РІР‚вЂќ Fixed Alembic revision generation by adding `backend/alembic/script.py.mako`; created baseline migration `3315ed698ecb_init.py`; applied `alembic upgrade head`. Verify: `SELECT * FROM alembic_version;` -> 3315ed698ecb.
...
2025-12-13 23:40 MSK Confirmed refactor didnРІР‚в„ўt break functionality: ran integration tests after moving usecase to domain port + updating repository; python -m pytest -q -> 1 passed.
...
2025-12-13 23:49 MSK Fixed Windows pytest РІР‚СљEvent loop is closedРІР‚Сњ for asyncpg/SQLAlchemy: added FastAPI lifespan in app/main.py to dispose SQLAlchemy async engine on shutdown; updated integration tests to use with TestClient(app); verified python -m pytest -q -> 2 passed
...
- 2025-12-14 10:13 MSK РІР‚вЂќ API contract updated (SSoT): added user personal blacklist by INN endpoints (/user/blacklist/inn GET/POST, /user/blacklist/inn/{inn} DELETE) and schemas AddUserBlacklistInnRequest, UserBlacklistInnListResponse (snake_case). Verified: YAML parsed via python yaml.safe_load => OK; Select-String confirms path + schema.


- 2025-12-14 09:49 MSK РїС—Р… Restored green test suite after recipients experiment (rollback broken alembic revision + fix repositories imports). Verify: .\.venv\Scripts\pytest.exe -q -> 11 passed; .\.venv\Scripts\python.exe -c "from app.adapters.db.repositories import RequestRepository; print('ok')" -> ok.
- 2025-12-14 09:57 MSK РїС—Р… Decision РїС—Р… UserMessaging recipients: PUT /user/requests/{requestId}/recipients uses replace-all semantics (server state mirrors UI checkboxes). Decision: when a domain is added to Blacklist, related suppliers must be automatically unselected (selected=false) across ALL requests to prevent sending. Verify (agreed): implement in Blacklist usecase + re-check in send usecase.
- 2025-12-14 10:08 MSK РїС—Р… Decision РїС—Р… New API schemas use snake_case (inn, supplier_id, created_at). User blacklist is personal and keyed by INN (UI shows company name + checko_data when available). Moderator blacklist stays global by domain (/moderator/blacklist/domains). Recipients PUT is replace-all; blacklist actions auto-unselect recipients accordingly.
 - 2025-12-14 10:49 MSK вЂ” Blacklist(User) routes wired: /api/v1/user/blacklist/inn (GET/POST) and /api/v1/user/blacklist/inn/{inn} (DELETE) are present in openapi.json; GET returns 501 Not Implemented. Verify: 	ry { Invoke-RestMethod -Method Get http://localhost:8000/api/v1/user/blacklist/inn } catch { $_.Exception.Response.StatusCode.value__ } -> 501; and (Invoke-RestMethod http://localhost:8000/openapi.json).paths... | Select-String '/api/v1/user/blacklist/inn' shows both paths.
